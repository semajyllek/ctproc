...
class CTProc:
    ...
    def get_proc_func(self) -> Callable:
        if self.config.is_topic:
            if self.config.trec_or_kz == 'trec':
                return self.process_trec_topic_data
            return self.process_kz_topic_data
    ...
    def process_trec_topic_data(self) -> Generator[None, None, CTTopic]:
        """
    ...
    def process_kz_topic_data(self) -> Generator[None, None, CTTopic]:
        """
        desc:       main method for processing a single pseudo-XML file of patient descriptions called "topics" in this sense
                    parameterized by CTConfig the by which the ClinProc object (self) was initialized
        returns:    yields processed CTTopic objects, one at a time
        """
        ...
        with open(self.config.data_path, 'r') as f:
            for line in f.readlines():
                ...
                if line.startswith('<NUM>'):
                    ...
                    text = line[7:].strip()
                    yield self.build_topic(topic_id, text)
